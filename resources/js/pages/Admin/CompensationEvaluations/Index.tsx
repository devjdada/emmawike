...
export default function CompensationEvaluationsIndex({
	auth,
	evaluations: initialEvaluations,
}: CompensationEvaluationsIndexProps) {
	...
	const filteredEvaluations = initialEvaluations.filter((evaluation) => {
		...
		const matchesType =
			typeFilter === "all" ||
			evaluation.evaluation_type.toLowerCase() === typeFilter.toLowerCase();
		return matchesSearch && matchesStatus && matchesType;
	});

	const handleDelete = (id: number) => {
		if (
			confirm("Are you sure you want to delete this compensation evaluation?")
		) {
								...
								<TableBody>
									{filteredEvaluations.map((evaluation) => (
										<TableRow key={evaluation.id}>
											...
											<TableCell>
												${evaluation.total_value?.toLocaleString() || "N/A"}
											</TableCell>
											<TableCell>
											...
											<TableCell>
												<div className="flex items-center gap-2">
													<Button variant="outline" size="sm" asChild>
														<a
															href={`${route(
																"admin.compensation-evaluations.show",
																evaluation.id,
															)}?print=true`}
															target="_blank"
														>
													...
													<Button variant="outline" size="sm" asChild>
														<Link
															href={route(
																"admin.compensation-evaluations.edit",
																evaluation.id,
															)}
														>
