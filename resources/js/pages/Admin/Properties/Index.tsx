...
export default function PropertiesIndex({ auth, properties: initialProperties }: PropertiesIndexProps) {
    ...
    const handleMediaUpload = (propertyId: string, files: File[], label: string) => {
        ...
        router.post(route('admin.properties.addMedia', propertyId), formData, {
            onSuccess: () => {
                toast({
    ...
    const handleDelete = (id: string) => {
        if (confirm('Are you sure you want to delete this property?')) {
            inertiaDelete(route('admin.properties.destroy', id), {
                onSuccess: () => {
                    toast({
    ...
    const handleStatusChange = (id: string, newStatus: string) => {
        router.put(
            route('admin.properties.updateStatus', id),
            { status: newStatus },
            {
                preserveState: true,
                preserveScroll: true,
    ...
    return (
        <AppLayout user={auth.user}>
                                        ...
                                        <TableRow key={property.id}>
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-2 text-sm">
                                                    <div className="h-4 w-4 text-muted-foreground">{property.currency}</div>
            ...
            <ImportPropertiesModal isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} />
        </AppLayout>
    );
}
