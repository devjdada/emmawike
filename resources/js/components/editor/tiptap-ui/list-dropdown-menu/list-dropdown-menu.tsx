...
// --- Tiptap UI ---
import { ListButton, canToggleList, isListActive, listOptions, type ListType } from '../list-button/list-button';
...
export function canToggleAnyList(editor: Editor | null, listTypes: ListType[]): boolean {
    ...
    return listTypes.some((type) => canToggleList(editor.editor, type));
}

export function isAnyListActive(editor: Editor | null, listTypes: ListType[]): boolean {
    if (!editor?.editor) return false;
    return listTypes.some((type) => isListActive(editor.editor, type));
}
...
export function shouldShowListDropdown(params: {
    ...
    if (!listInSchema || !editor?.editor) {
        return false;
    }

    if (hideWhenUnavailable) {
        if (isNodeSelection(editor.editor.state.selection) || !canToggleAny) {
            return false;
        }
    }
...
export function useListDropdownState(editor: Editor | null, availableTypes: ListType[]) {
    ...
    const listInSchema = availableTypes.some((type) => isNodeInSchema(type, editor?.editor));
...
export function useActiveListIcon(editor: Editor | null, filteredLists: typeof listOptions) {
    return React.useCallback(() => {
        const activeOption = filteredLists.find((option) => isListActive(editor?.editor, option.type));
