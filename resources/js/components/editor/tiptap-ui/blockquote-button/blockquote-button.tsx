...
// --- Icons ---
import { BlockQuoteIcon } from '@/components/editor/tiptap-icons/block-quote-icon';
...
export function canToggleBlockquote(editor: Editor | null): boolean {
    ...
    try {
        return editor.editor.can().toggleWrap('blockquote');
    } catch {
        return false;
    }
}
...
export function isBlockquoteActive(editor: Editor | null): boolean {
    if (!editor?.editor) return false;
    return editor.editor.isActive('blockquote');
}

export function toggleBlockquote(editor: Editor | null): boolean {
    if (!editor?.editor) return false;
    return editor.editor.chain().focus().toggleWrap('blockquote').run();
}

export function isBlockquoteButtonDisabled(editor: Editor | null, canToggle: boolean, userDisabled: boolean = false): boolean {
    if (!editor?.editor) return true;
...
export function shouldShowBlockquoteButton(params: {
    ...
    if (!nodeInSchema || !editor?.editor) {
        return false;
    }

    if (hideWhenUnavailable) {
        if (isNodeSelection(editor.editor.state.selection) || !canToggle) {
            return false;
        }
    }

    return Boolean(editor?.editor?.isEditable);
}

export function useBlockquoteState(editor: Editor | null, disabled: boolean = false, hideWhenUnavailable: boolean = false) {
    const nodeInSchema = isNodeInSchema('blockquote', editor?.editor);
...
export const BlockquoteButton = React.forwardRef<HTMLButtonElement, BlockquoteButtonProps>(
    ({ editor: providedEditor, text, hideWhenUnavailable = false, className = '', disabled, onClick, children, ...buttonProps }, ref) => {
        const { editor, canCommand } = useTiptapEditor(providedEditor);

        const { isDisabled, isActive, shouldShow, handleToggle, shortcutKey, label } = useBlockquoteState(editor, disabled, hideWhenUnavailable);
        ...
        if (!shouldShow || !editor?.editor || !editor.editor.isEditable) {
            return null;
        }
