...
// --- Tiptap UI ---
import { ColorHighlightButton, canToggleHighlight } from '../color-highlight-button/color-highlight-button';
...
export const ColorHighlightPopoverButton = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, children, ...props }, ref) => (
    <Button
        type="button"
        className={className}
        data-style="ghost"
...
export function ColorHighlightPopoverContent({
    ...
    const removeHighlight = React.useCallback(() => {
        ...
        editor.editor.chain().focus().unsetMark('highlight').run();
        onClose?.();
    }, [editor, onClose]);
...
export function ColorHighlightPopover({
    ...
    const markAvailable = isMarkInSchema('highlight', editor?.editor);
    ...
    React.useEffect(() => {
        ...
        const updateIsDisabled = () => {
            ...
            if (!markAvailable || !editor?.editor) {
                isDisabled = true;
            }

            const isInCompatibleContext = editor.editor.isActive('code') || editor.editor.isActive('codeBlock') || editor.editor.isActive('imageUpload');
        ...
        editor.editor.on('selectionUpdate', updateIsDisabled);
        editor.editor.on('update', updateIsDisabled);

        return () => {
            editor.editor.off('selectionUpdate', updateIsDisabled);
            editor.editor.off('update', updateIsDisabled);
        };
    }, [editor, markAvailable]);

    const isActive = editor?.editor?.isActive('highlight') ?? false;

    const shouldShow = React.useMemo(() => {
        if (!hideWhenUnavailable || !editor?.editor) return true;

        return !(isNodeSelection(editor.editor.state.selection) || !canToggleHighlight(editor.editor));
