...
export function canToggleMark(editor: Editor | null, type: Mark): boolean {
    ...
    try {
        return editor.editor.can().toggleMark(type);
    } catch {
        return false;
    }
}
...
export function toggleMark(editor: Editor | null, type: Mark): void {
    ...
    editor.editor.chain().focus().toggleMark(type).run();
}
...
export function shouldShowMarkButton(params: {
    ...
    if (hideWhenUnavailable) {
        if (isNodeSelection(editor.editor.state.selection) || !canToggleMark(editor, type)) {
            return false;
        }
    }
...
export function useMarkState(editor: Editor | null, type: Mark, disabled: boolean = false) {
    const markInSchema = isMarkInSchema(type, editor?.editor);
    ...
    ({ editor: providedEditor, type, text, hideWhenUnavailable = false, className = '', disabled, onClick, children, ...buttonProps }, ref) => {
        ...
        const handleClick = React.useCallback(
            (e: React.MouseEvent<HTMLButtonElement>) => {
                ...
                if (!e.defaultPrevented && !isDisabled && editor?.editor) {
