...
export function canToggleCodeBlock(editor: Editor | null): boolean {
    ...
    try {
        return editor.editor.can().toggleNode('codeBlock', 'paragraph');
    } catch {
        return false;
    }
}
...
export function isCodeBlockActive(editor: Editor | null): boolean {
    if (!editor?.editor) return false;
    return editor.editor.isActive('codeBlock');
}

export function toggleCodeBlock(editor: Editor | null): boolean {
    if (!editor?.editor) return false;
    return editor.editor.chain().focus().toggleNode('codeBlock', 'paragraph').run();
}

export function isCodeBlockButtonDisabled(editor: Editor | null, canToggle: boolean, userDisabled: boolean = false): boolean {
...
export function useCodeBlockState(editor: Editor | null, disabled: boolean = false, hideWhenUnavailable: boolean = false) {
    const nodeInSchema = isNodeInSchema('codeBlock', editor?.editor);
...
export const CodeBlockButton = React.forwardRef<HTMLButtonElement, CodeBlockButtonProps>(
    ({ editor: providedEditor, text, hideWhenUnavailable = false, className = '', disabled, onClick, children, ...buttonProps }, ref) => {
        ...
        const { isDisabled, isActive, shouldShow, handleToggle, shortcutKey, label } = useCodeBlockState(editor, disabled, hideWhenUnavailable);
