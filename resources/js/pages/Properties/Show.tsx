            ...
            <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
                <div className="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                    <div className="p-6">
                        ...
                        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{property.address_line1}, {property.city}, {property.state}</p>

                        <div className="mt-6">
                            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Property Details</h2>
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <p><span className="font-semibold">Price:</span> {property.currency} {property.price.toLocaleString()}</p>
                                ...
                                <div>
                                    ...
                                    <p><span className="font-semibold">Area:</span> {property.area_sq_ft} sq. ft.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-6">
                            ...
                            <div className="mt-4 prose dark:prose-invert" dangerouslySetInnerHTML={{ __html: property.description }} />
                        </div>
                        ...
                        <div className="mt-6">
                            ...
                            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                {property.media.map((media_item) => (
                                    <div key={media_item.id} className="overflow-hidden rounded-lg">
                                        {media_item.type === 'image' ? (
                                            <img src={`/storage/${media_item.path}`} alt={media_item.label || property.title} className="w-full h-full object-cover" />
                                        ) : (
                                            <video src={`/storage/${media_item.path}`} controls className="w-full h-full" />
                                        )}
                                    </div>
                                ))}
                            </div>
